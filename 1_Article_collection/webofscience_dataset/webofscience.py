...
def Article_filter_duplicate():
    df = pd.read_csv("ReRAM_DB_Keyword_Date_Document.csv")
    df.duplicated(['title'],keep=False).to_csv("Duplicated.csv")
    df_new = df.drop_duplicates(['title'], keep='first', ignore_index=True)

    df_new.to_csv("ReRAM_DB_Duplicate.csv", index=False)
